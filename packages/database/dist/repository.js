...
export class DatabaseRepository {
    static async saveCommit(commit) {
        const result = await pool.query(`INSERT INTO commits (repository, branch, commit_sha, previous_commit_sha, author, message, timestamp)
    ...
    static async getCommit(commitSha) {
        const result = await pool.query(`SELECT id, repository, branch, commit_sha as "commitSha", previous_commit_sha as "previousCommitSha", 
              author, message, timestamp, created_at as "createdAt"
       FROM commits WHERE commit_sha = $1`, [commitSha]);
        return result.rows[0] || null;
    }
    static async saveBuild(build) {
        const result = await pool.query(`INSERT INTO builds (commit_id, build_id, previous_build_id, status, bundle_size, dependency_delta)
       VALUES ($1, $2, $3, $4, $5, $6)
    ...
    static async saveDeploymentReport(report) {
        const result = await pool.query(`INSERT INTO deployment_reports (commit_sha, previous_commit_sha, build_id, previous_build_id, 
    ...
    static async saveTestRun(run) {
        const result = await pool.query(`INSERT INTO test_runs (report_id, test_id, spec_file, status, duration_ms)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, started_at as "startedAt"`, [run.reportId, run.testId, run.specFile, run.status, run.durationMs]);
        return { ...run, id: result.rows[0].id, startedAt: result.rows[0].startedAt, completedAt: undefined, durationMs: run.durationMs };
    }
    static async updateTestRunStatus(testRunId, status, durationMs) {
        const completedAt = status === 'passed' || status === 'failed' || status === 'skipped' ? new Date().toISOString() : undefined;
        await pool.query(`UPDATE test_runs SET status = $1, duration_ms = $2, completed_at = $3 WHERE id = $4`, [status, durationMs, completedAt, testRunId]);
    }
    static async saveArtifact(artifact) {
        const result = await pool.query(`INSERT INTO test_artifacts (test_run_id, type, url) VALUES ($1, $2, $3)
    ...
    static async saveRootCause(insight) {
        const result = await pool.query(`INSERT INTO root_cause_insights (test_run_id, failure_summary, root_cause, suggested_fix, 
    ...
    static async getReports(limit = 50) {
        const result = await pool.query(`SELECT id, commit_sha as "commitSha", previous_commit_sha as "previousCommitSha", 
    ...
    static async getTestRuns(reportId) {
        const result = await pool.query(`SELECT id, report_id as "reportId", test_id as "testId", spec_file as "specFile",
    ...
    static async getArtifacts(testRunId) {
        const result = await pool.query(`SELECT id, test_run_id as "testRunId", type, url, created_at as "timestamp"
       FROM test_artifacts WHERE test_run_id = $1`, [testRunId]);
        return result.rows;
    }
    static async getRootCause(testRunId) {
        const result = await pool.query(`SELECT id, test_run_id as "testRunId", failure_summary as "failureSummary", 
