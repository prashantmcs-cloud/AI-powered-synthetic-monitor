import { createAiTestSpec } from '@ai-synthetic/shared-types';
import { writeFileSync, mkdirSync } from 'node:fs';
import { join } from 'node:path';
export function analyzeCommitChanges(comparison) {
    const { currentCommit, previousCommit } = comparison;
    const changedFiles = currentCommit.files;
    const flows = [];
    const rationaleParts = [];
    const uiRelatedFiles = changedFiles.filter(f => f.includes('component') || f.includes('page') || f.includes('.tsx') || f.includes('.jsx') || f.includes('.vue'));
    const apiRelatedFiles = changedFiles.filter(f => f.includes('api') || f.includes('route') || f.includes('controller') || f.includes('service'));
    const authRelatedFiles = changedFiles.filter(f => f.includes('auth') || f.includes('login') || f.includes('signin') || f.includes('middleware'));
    const checkoutRelatedFiles = changedFiles.filter(f => f.includes('checkout') || f.includes('cart') || f.includes('payment') || f.includes('order'));
...
export function generatePlaywrightTests(comparison, build, affectedFlows) {
    const specs = [];
    const repoSlug = comparison.currentCommit.sha.substring(0, 7);
    for (const flow of affectedFlows) {
        const spec = createAiTestSpec({
...
function generateStepsForFlow(flow, bundleSize) {
    const baseSteps = [
...
function generatePlaywrightSpecContent(spec) {
...
export function writeSpecFiles(specs) {
